...
const LoginPage = ({ onSignUpClick, onForgotPasswordClick, onBackClick }) => {
  ...
  // This state stores checkbox value for "keep signed in".
  ...
  // This state stores any login error message.
  const [error, setError] = useState('');
  const socialToken = searchParams.get('token');
  const loginUrl = `${API_BASE_URL}/api/auth/login`;

  // This helper sends user to social auth route.
  const redirectToSocialLogin = (provider) => {
    window.location.href = `${API_BASE_URL}/api/auth/${provider}`;
  };

  // Social auth redirects back to `/login?token=...`; store it and send user to home.
  useEffect(() => {
    ...
    const handleSocialLogin = async () => {
      try {
        setStoredAuth({ token: socialToken });
        const me = await fetchMe(socialToken);
        setStoredAuth({ token: socialToken, user: me });
    ...
    handleSocialLogin();
  }, [navigate, socialToken]);

  const handleLogin = async (e) => {
    // This function handles login form submit.
    e.preventDefault();
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate(data.redirectTo || getDashboardPathForRole(data.user?.role), { replace: true });
      } else {
      ...
      <div style={styles.formContainer} className="w-full max-w-md">
        ...
        <div className="space-y-3 mb-8">
          ...
          <button 
            ...
            onClick={() => redirectToSocialLogin('apple')}
