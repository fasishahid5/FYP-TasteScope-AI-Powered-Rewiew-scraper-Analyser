...
const ForgotPasswordPage = ({ onBackClick }) => {
  // This state stores the email input value.
  const [email, setEmail] = useState('');

  // This state stores error and success messages.
  const [error, setError] = useState('');
  const [message, setMessage] = useState('');
  const forgotPasswordUrl = `${API_BASE_URL}/api/auth/forgot-password`;

  // This function sends reset-link request to backend.
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.msg || 'Request failed');
      }
    } catch (err) {
...
const styles = {
  ...
  formContainer: {
    // This card contains the form content.
